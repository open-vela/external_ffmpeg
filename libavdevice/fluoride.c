...
#define FLUORIDE_CTRL_PATH         "/data/misc/bluedroid/.fluoride_ctrl"
#define FLUORIDE_DATA_PATH         "/data/misc/bluedroid/.fluoride_data"
...
static int ff_fluoride_send_command(FluPriv *priv, FLUORIDE_CTRL_CMD cmd)
{
    ...
    if (priv->ctrl_fd == FLUORIDE_AUDIO_DISCONNECTED){
        ret = ff_fluoride_open_path(priv,FLUORIDE_CTRL_PATH);
        if (ret < 0)
...
static int ff_fluoride_open_path(FluPriv *priv, const char *path)
{
    ...
    if (ret > 0) {
        if (!strcmp(path, FLUORIDE_CTRL_PATH)) {
            priv->ctrl_fd = ret;
            ret = ff_fluoride_send_command(priv, FLUORIDE_CTRL_CMD_CHECK_READY);
        } else if (!strcmp(path, FLUORIDE_DATA_PATH)) {
            priv->data_fd = ret;
        }
    }
...
{
    if ((ff_fluoride_open_path(priv, FLUORIDE_CTRL_PATH)) < 0 ||
        (ff_fluoride_open_path(priv, FLUORIDE_DATA_PATH)) < 0)
        return -1;
...
int ff_fluoride_write_buffer(FluPriv *priv, void* buffer, size_t bytes)
{
    ...
    if (priv->data_fd == FLUORIDE_AUDIO_DISCONNECTED) {
        ret = ff_fluoride_open_path(priv,FLUORIDE_DATA_PATH);
        if (ret < 0)
            return AVERROR(EAGAIN);
    }
...
{
    fluoride_btav_codec_index_t codec_type;
    fluoride_btav_codec_sample_rate_t sample_rate;
    fluoride_btav_codec_channel_mode_t channel_mode;
    fluoride_btav_codec_bits_per_sample_t bits_per_sample;
    uint32_t bit_rate;
    int ret;
...
int ff_fluoride_open(AVFormatContext *ctx)
{
    ...
    if (!priv->playback)
        ...
        } else if (priv->state == FLUORIDE_STATE_STARTING)
            return AVERROR(EAGAIN);
    }
