...
static int amoviesink_async_interrupt(void *opaque)
{
    ...
    pthread_mutex_lock(&priv->mutex);
    for (entry = dq_peek(&priv->cmd_queue); entry; entry = dq_next(entry)) {

        msg = (MovieSinkCmd *)entry;
...
static int amoviesink_proc_dat(AVFilterContext *ctx)
{
    ...
    if (priv->state != AVMOVIE_ASYNC_STATE_STARTED) {
        ...
        return priv->state == AVMOVIE_ASYNC_STATE_PAUSED ? 0 : AVERROR(EPERM);
    }
