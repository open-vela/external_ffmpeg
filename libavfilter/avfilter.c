...
static int link_init_dump_pcm(AVFilterLink *link)
{
    ...
    link->nb_dump_pcm_fds = av_sample_fmt_is_planar(link->format)? link->channels : 1;
    link->dump_pcm_fds = av_mallocz_array(link->nb_dump_pcm_fds, sizeof(int));
    if (!link->dump_pcm_fds)
        return AVERROR(ENOMEM);
...
void ff_filter_set_ready(AVFilterContext *filter, unsigned priority)
{
    ...
    if (filter->graph && filter->graph->ready)
        filter->graph->ready(filter);
}
...
static int link_dump_frame(AVFilterLink *link, AVFrame *frame)
{
    ...
    if (av_sample_fmt_is_planar(frame->format)) {
        int i;
        for (i = 0; i < link->nb_dump_pcm_fds && i < frame->channels; i++) {
        ...
        ret = write(link->dump_pcm_fds[0], frame->data[0], samples_size * frame->channels);
        if (ret < 0)
            goto err;
    }
