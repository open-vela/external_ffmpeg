...
static int movie_async_output_props(AVFilterLink *outlink)
{
    ...
    switch (outlink->type) {
        case AVMEDIA_TYPE_VIDEO:
            outlink->w = movie->streams[out_id].frame->width;
            outlink->h = movie->streams[out_id].frame->height;
            break;
...
static int movie_async_query_formats(AVFilterContext *ctx)
{
    ...
    for (i = 0; i < ctx->nb_outputs; i++) {
        if (!movie->streams[i].frame)
            continue;

        flags   = true;
        outlink = ctx->outputs[i];
        frame   = movie->streams[i].frame;
...
static int movie_async_activate(AVFilterContext *ctx)
{
    ...
    for (i = 0; i < ctx->nb_outputs; i++) {
        ...
        if (movie->streams[i].framed == 0) {
            movie->streams[i].frame = movie_async_recv_dat(ctx, i);
...
static int movie_async_dump(AVFilterContext *ctx, char *res, int res_len)
{
    ...
    int pos = 0, ret, i;
     ...
     ret = snprintf(res, res_len, "st: %d br: %lld", movie->state,
                                  movie->format_ctx ? movie->format_ctx->bit_rate : 0);
     pos += ret;
