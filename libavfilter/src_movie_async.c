...
{
    return movie->streams[pad_id].st == NULL ||
...
static int movie_async_open_decoder(AVFilterContext *ctx, MovieStream *stream)
{
    AVCodec *codec;
    int ret;

    codec = avcodec_find_decoder(stream->st->codecpar->codec_id);
    if (!codec) {
    ...
    ret = avcodec_parameters_to_context(stream->codec_ctx, stream->st->codecpar);
    if (ret < 0)
        return ret;
...
static int movie_async_open_demuxer(AVFilterContext *ctx, const char *filename)
{
    ...
    for (i = 0; i < ctx->nb_outputs; i++) {
        ...
        movie->streams[i].st = movie->format_ctx->streams[ret];
        ret = movie_async_open_decoder(ctx, &movie->streams[i]);
        if (ret < 0)
            goto out;
        ...
        if (movie->streams[i].codec_ctx->codec_type == AVMEDIA_TYPE_AUDIO &&
            ...
            movie->streams[i].codec_ctx->channel_layout =
                av_get_default_channel_layout(movie->streams[i].st->codecpar->channels);
        }
    }
...
static int movie_async_read_frame(AVFilterContext *ctx)
{
    ...
    for (i = 0; i < ctx->nb_outputs; i++) {
        ...
        if (!movie_async_output_inactive(movie, i) &&
            pkt.stream_index == movie->streams[i].st->index) {
...
static int movie_async_dec_frame(AVFilterContext *ctx, int pad_id, AVFrame **oframe)
{
    ...
    movie->current_ms = frame->pts * av_q2d(movie->streams[pad_id].st->time_base) * 1000;
...
static int movie_async_output_props(AVFilterLink *outlink)
{
    ...
    MovieStream *st = &movie->streams[out_id];
...
static int movie_async_query_formats(AVFilterContext *ctx)
{
    ...
    for (i = 0; i < ctx->nb_outputs; i++) {
        ...
        frame   = ff_framequeue_peek(&movie->streams[i].dat_queue, 0);

        switch (outlink->type) {
            case AVMEDIA_TYPE_AUDIO:
                list[0] = frame->sample_rate;
                if ((ret = ff_formats_ref(ff_make_format_list(list), &outlink->in_samplerates)) < 0)
                    return ret;

                list64[0] = frame->channel_layout;
...
{
    MovieAsyncContext *movie = ctx->priv;
    int status, i, ret = FFERROR_NOT_READY;
    AVFilterLink *link;
    AVFrame *frame;
                                       ...
                                       char *res, int res_len, int flags)
{
    ...
    if (!strcmp(cmd, "open")) {
        ...
        for (i = 0; i < ctx->nb_outputs; i++) {
            ...
            if (movie->streams[i].codec_ctx->codec_type == AVMEDIA_TYPE_AUDIO) {
                ret = snprintf(res + pos, res_len - pos, ", A: %d %s %d %d %d",
                                        movie->streams[i].st->index,
            ...
            } else if (movie->streams[i].codec_ctx->codec_type == AVMEDIA_TYPE_VIDEO) {
