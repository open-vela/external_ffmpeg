...
static int query_stats(AVFilterContext *ctx, const char *cmd, char *res, int res_len)
{
    ...
    if (!strcmp(cmd, "lavfi.astats")) {
        char *buffer = NULL;

        if ((ret = av_dict_get_string(metadata, &buffer, '=', ';')) >= 0) {
            av_strlcpy(res, buffer, res_len);
            av_freep(&buffer);
        }
    } else {
        AVDictionaryEntry *tag = av_dict_get(metadata, cmd, NULL, 0);
        if (tag != NULL)
            av_strlcpy(res, tag->value, res_len);
        else
            ret = AVERROR(EINVAL);
    }
                           ...
                           char *res, int res_len, int flags)
{
    ...
    if (!strncmp(cmd, "lavfi.astats", strlen("lavfi.astats")))
