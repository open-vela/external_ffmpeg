...
static int filter_ref_agreed_formats(AVFilterContext *ctx)
{
    ...
    for (i = 0; i < ctx->nb_inputs; i++) {
        if (!ctx->inputs[i]->outcfg.formats) {
            ...
            ret = ff_formats_ref(rates, &ctx->inputs[i]->outcfg.samplerates);
            if (ret < 0)
                return ret;

            ret = ff_channel_layouts_ref(layouts, &ctx->inputs[i]->outcfg.channel_layouts);
            if (ret < 0)
                return ret;
        }
    }
    ...
    for (i = 0; i < ctx->nb_outputs; i++) {
        if (!ctx->outputs[i]->incfg.formats) {
            ...
            ret = ff_formats_ref(rates, &ctx->outputs[i]->incfg.samplerates);
            if (ret < 0)
                return ret;

            ret = ff_channel_layouts_ref(layouts, &ctx->outputs[i]->incfg.channel_layouts);
            if (ret < 0)
                return ret;
        }
    }
