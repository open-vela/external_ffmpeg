...
static int filter_ref_agreed_formats(AVFilterContext *ctx)
{
    ...
    if (!formats) {
        for (i = 0; i < ctx->nb_outputs; i++) {
            if (ctx->outputs[i]->in_formats) {
                formats = ctx->outputs[i]->in_formats;
                rates   = ctx->outputs[i]->in_samplerates;
                layouts = ctx->outputs[i]->in_channel_layouts;
                break;
            }
        }
    }
    ...
    for (i = 0; i < ctx->nb_inputs; i++) {
        if (!ctx->inputs[i]->out_formats) {
            ret = ff_formats_ref(formats, &ctx->inputs[i]->out_formats);
            if (ret < 0)
                return ret;

            ret = ff_formats_ref(rates, &ctx->inputs[i]->out_samplerates);
            if (ret < 0)
                return ret;
    ...
    for (i = 0; i < ctx->nb_outputs; i++) {
        if (!ctx->outputs[i]->in_formats) {
            ret = ff_formats_ref(formats, &ctx->outputs[i]->in_formats);
            if (ret < 0)
                return ret;

            ret = ff_formats_ref(rates, &ctx->outputs[i]->in_samplerates);
            if (ret < 0)
                return ret;

            ret = ff_channel_layouts_ref(layouts, &ctx->outputs[i]->in_channel_layouts);
            if (ret < 0)
                return ret;
        }
    }
...
static int filter_query_formats(AVFilterContext *ctx)
{
    ...
    if ((ret = filter_ref_agreed_formats(ctx)) >= 0)
        return ret;
    ...
    /* Don't need full fill the formats when sanitize_formats setting */
    if (ctx->filter->sanitize_formats)
        return 0;
...
static bool formats_useless(AVFilterContext *f)
{
    ...
    if (f->nb_inputs == 0 || f->nb_outputs == 0)
        return false;
...
static bool sanitize_formats(AVFilterGraph *graph)
{
    ...
    for (i = 0; i < graph->nb_filters; i++) {
        ...
        for (j = 0; j < f->nb_inputs; j++) {
            if (!f->inputs[j]->out_formats && f->inputs[j]->in_formats) {
        ...
        for (j = 0; j < f->nb_outputs; j++) {
            if (!f->outputs[j]->in_formats && f->outputs[j]->out_formats) {
                ff_avfilter_link_unref_formats(f->outputs[j]);
                changed = true;
            }
        }

        if (f->filter->sanitize_formats) {
            if (f->filter->sanitize_formats(f) == AVERROR(EAGAIN))
                changed = true;
        } else if (formats_useless(f)) {
            for (j = 0; j < f->nb_inputs; j++)
                ff_avfilter_link_unref_formats(f->inputs[j]);
...
#undef MERGE_DISPATCH
            ...
            if (convert_needed && count_delayed == old_count_delayed) {
                ...
                case AVMEDIA_TYPE_VIDEO:
                    ...
                    snprintf(inst_name, sizeof(inst_name), "auto_scaler");
                ...
                case AVMEDIA_TYPE_AUDIO:
                    ...
                    snprintf(inst_name, sizeof(inst_name), "auto_resampler");
...
static int reduce_formats_on_filter(AVFilterContext *filter)
{
    ...
    for (i = 0; i < filter->nb_inputs; i++) {
        ...
        for (j = 0; j < filter->nb_outputs; j++) {
            ...
            if (!fmts || inlink->type != outlink->type || fmts->nb_channel_layouts == 1)
                continue;
...
static int pick_formats(AVFilterGraph *graph)
{
    ...
    do{
        ...
        for (i = 0; i < graph->nb_filters; i++) {
            ...
            if (filter->nb_inputs){
                for (j = 0; j < filter->nb_inputs; j++){
                    if(filter->inputs[j]->sample_rate <= 0 &&
                       filter->inputs[j]->in_formats && filter->inputs[j]->in_formats->nb_formats == 1) {
            ...
            if (filter->nb_outputs){
                for (j = 0; j < filter->nb_outputs; j++){
                    if(filter->outputs[j]->sample_rate <= 0 &&
                       filter->outputs[j]->in_formats && filter->outputs[j]->in_formats->nb_formats == 1) {
            ...
            if (filter->nb_inputs && filter->nb_outputs && filter->inputs[0]->format>=0) {
                for (j = 0; j < filter->nb_outputs; j++) {
                    if(filter->outputs[j]->format<0 && filter->outputs[j]->in_formats) {
...
static void graph_clear_formats(AVFilterGraph *graph, void *log_ctx)
{
    ...
    for (i = graph->nb_filters - 1; i >= 0; i--) {
        ...
        if ((!strcmp(f->filter->name, "aresample") && !strcmp(f->name, "auto_resampler")) ||
            (!strcmp(f->filter->name, "scale") && !strcmp(f->name, "auto_scaler"))) {

            if (f->inputs[0]->status_out || f->outputs[0]->status_in) {
        ...
        for (j = 0; j < f->nb_inputs; j++) {
            if (f->inputs[j]->status_out) {
                ff_formats_unref(&f->inputs[j]->out_formats);
                ff_formats_unref(&f->inputs[j]->out_samplerates);
                ff_channel_layouts_unref(&f->inputs[j]->out_channel_layouts);

                if (f->inputs[j]->status_in) {
                    f->inputs[j]->format         = -1;
                    f->inputs[j]->channel_layout = 0;
                    f->inputs[j]->channels       = 0;
        ...
        for (j = 0; j < f->nb_outputs; j++) {
            if (f->outputs[j]->status_in) {
                ff_formats_unref(&f->outputs[j]->in_formats);
                ff_formats_unref(&f->outputs[j]->in_samplerates);
                ff_channel_layouts_unref(&f->outputs[j]->in_channel_layouts);

                if (f->outputs[j]->status_out) {
                    f->outputs[j]->format         = -1;
                    f->outputs[j]->channel_layout = 0;
                    f->outputs[j]->channels       = 0;
